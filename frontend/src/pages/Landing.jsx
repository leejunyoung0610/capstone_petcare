    ...
    <div className="min-h-screen bg-slate-50 bg-dot-slate bg-[length:28px_28px]">
          ...
          <div className="grid items-center gap-14 lg:grid-cols-2 lg:gap-16">
            <div className="max-w-xl lg:max-w-none">
              ...
              <p className="prose prose-slate prose-lg mt-6 max-w-xl text-pretty text-slate-600 prose-p:leading-relaxed">
                ...
                으로 여러 안구 질환을 참고용으로 스크리닝합니다. 결과는 PDF로 정리해 병원 방문 시 활용할 수 있습니다.
              </p>
              <ul className="mt-8 flex flex-col gap-3 text-sm text-slate-600 sm:flex-row sm:flex-wrap sm:gap-x-8">
                <li className="flex items-center gap-2">
                  <span className="flex size-6 items-center justify-center rounded-full bg-emerald-100 text-emerald-700">✓</span>
                  회원가입 후 반려동물 등록
                </li>
                <li className="flex items-center gap-2">
                  <span className="flex size-6 items-center justify-center rounded-full bg-emerald-100 text-emerald-700">✓</span>
              ...
              <div className="mt-10 flex flex-wrap gap-4">
                <Link to="/register">
                  <button className="inline-flex items-center gap-2 px-8 py-3.5 bg-blue-600 text-white rounded-xl text-base font-semibold hover:bg-blue-700 shadow-md">
                    <Upload className="size-5" />
                    무료로 시작하기
                  </button>
                </Link>
                <button className="cursor-not-allowed px-8 py-3.5 border border-slate-300 rounded-xl text-base text-slate-400 opacity-55" disabled>
                  질환백과 보기
                </button>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={t} className="group rounded-2xl border border-slate-200/90 bg-gradient-to-b from-white to-slate-50/80 p-7 shadow-sm transition duration-300 hover:-translate-y-0.5 hover:border-blue-200/80 hover:shadow-float">
                <div className="font-display text-3xl font-bold tracking-tight text-brand-link md:text-4xl">{n}</div>
            ...
            {[
              {
                ...
                title: "멀티 라벨 AI 분석",
                desc: "한 장의 이미지로 여러 안구 이상 가능성을 동시에 참고합니다. 최종 판단은 반드시 수의사에게 맡기세요.",
              },
              {
                ...
                title: "리포트 · 병원 연계",
                desc: "PDF 보고서로 소견을 정리해 두면 진료 시 설명이 수월합니다. 원격 수의사 소견은 순차 오픈 예정입니다.",
              },
              {
                ...
                title: "반려동물별 기록",
                desc: "여러 마리를 등록하고, 진단 이력을 계정에 남겨 두었을 때 변화 추이를 가족이 함께 확인할 수 있습니다.",
              },
            ].map((item) => (
              <div key={item.title} className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                <div className={`mx-auto mb-5 flex size-14 items-center justify-center rounded-2xl bg-gradient-to-br ${item.color} text-white shadow-lg ${item.shadow} md:mx-0`}>
                  {item.icon}
                </div>
                <h3 className="font-display text-xl font-bold text-slate-900">{item.title}</h3>
                <p className="mt-3 text-sm leading-relaxed text-slate-600 md:text-[15px] md:leading-7">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={item.step} className="relative rounded-2xl border border-slate-200/90 bg-white p-7 shadow-sm transition hover:shadow-md">
                <div className="absolute right-5 top-5 font-mono text-5xl font-bold text-slate-100">{item.step}</div>
                <div className="relative mb-5 flex size-12 items-center justify-center rounded-xl bg-brand-link text-lg font-bold text-white shadow-md">{item.step}</div>
            ...
            <div className="min-w-0">
              ...
              <div className="prose prose-slate prose-sm mt-3 max-w-none text-slate-700">
                <p>
                  ...
                  <strong>진단이 아닌 사전 스크리닝·참고 정보</strong>입니다. 치료 결정은 반드시 면허 수의사와 상담하시기 바랍니다.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 py-20 text-white md:py-24">
        ...
        <div className="relative mx-auto max-w-3xl px-4 text-center sm:px-6">
          ...
          <div className="mt-10">
            <Link to="/register">
              <button className="inline-flex items-center gap-2 border border-white/90 bg-white px-10 py-4 text-base font-semibold text-blue-700 rounded-xl shadow-lg hover:bg-blue-50">
                <Upload className="size-5" />
                무료로 시작하기
              </button>
            </Link>
      ...
      <footer className="bg-zinc-950 py-14 text-zinc-400">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-10 grid gap-10 md:grid-cols-4 md:gap-8">
            ...
            <div>
              <h4 className="font-display text-sm font-semibold uppercase tracking-wider text-zinc-300">서비스</h4>
              <ul className="mt-4 space-y-2.5 text-sm">
                <li><Link to="/register" className="transition hover:text-white">AI 스크리닝</Link></li>
                <li><span className="cursor-not-allowed opacity-50">수의사 찾기</span></li>
                <li><span className="cursor-not-allowed opacity-50">질환백과</span></li>
              </ul>
            </div>
            <div>
              <h4 className="font-display text-sm font-semibold uppercase tracking-wider text-zinc-300">지원</h4>
              <ul className="mt-4 space-y-2.5 text-sm">
                <li><span className="cursor-not-allowed opacity-50">공지사항</span></li>
                <li><span className="cursor-not-allowed opacity-50">FAQ</span></li>
                <li><span className="cursor-not-allowed opacity-50">이용약관</span></li>
              </ul>
            </div>
            <div>
              <h4 className="font-display text-sm font-semibold uppercase tracking-wider text-zinc-300">문의</h4>
              <ul className="mt-4 space-y-2.5 text-sm">
