...
> 관련 파일: `backend/docker-compose.prod.yml`, `backend/nginx/nginx.prod.conf`, `backend/.env.example`, `frontend/.env.example`
...
- Nginx 프로덕션 CSP·HSTS (`backend/nginx/nginx.prod.conf`)
...
- 종류별 메일 템플릿 (`backend/app/services/mail_templates.py`)
- SMTP 발송 (`backend/app/core/email.py`)
...
### 환경변수·시크릿 (`backend/.env` — **저장소 커밋 금지**)
...
- [ ] `python backend/scripts/test_smtp.py` 발송 테스트
- [ ] 비밀번호 재설정 메일 E2E
